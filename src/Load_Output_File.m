...
%% ######################### LOAD INPUT FILE ################################
%  ##########################################################################
function succeed = Load_Output_File(handles)
try
    %--------------------------------------------------------------------------
    ...
    fid = fopen(myData.workdir_eeDAPoutputFile);
...
%     end
    ...
    % skip the task_start and task_finish
    tline = fgets(fid);   
    tline = fgets(fid);
    ...
    while ~feof(fid)
        ...
        taskinfo.id = char(temp{2});
        taskinfo.order = str2double(temp{3});
        taskinfo.slot = str2double(temp{4});
        taskinfo.upleftX = str2double(temp{end-5});
        taskinfo.upleftY = str2double(temp{end-4});
        taskinfo.bottomleftX = str2double(temp{end-3});
        taskinfo.bottomleftY = str2double(temp{end-2});
        taskinfo.bottomrightX = str2double(temp{end-1});
        taskinfo.bottomrightY = str2double(temp{end});
        handles.myData.taskinfo = taskinfo;

        eedapOutFileInfo{ntasks} = handles.myData.taskinfo; %#ok<AGROW>
    ...
    myData.eedapOutFileInfo = eedapOutFileInfo;
   % myData.tasks_out = tasks_out;
    
    % Pack myData into handles
    % Update handles.Registration_after_work
    handles.myData=myData;
    guidata(handles.Registration_after_work,handles);
    succeed = 1;
catch ME
